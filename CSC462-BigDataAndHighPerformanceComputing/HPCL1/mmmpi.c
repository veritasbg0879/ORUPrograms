...
double dotProduct(double *a, double *b) {
    ...
    // Note: comparing int and size_t is fine as long as int is 0 or positive
    for (i = 0; i < n; i++) {
        dp += a[i] * b[i];
    }
...
int main(int argc, char** argv) {
    ...
    //Broadcast send/recv the B Matrix
    MPI_Bcast(B[0], n*n, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    // Should change this to send and recv portions

	...
	for (int i = 0; i < vCount; i++) {
		...
		for (int j = 0; j < vCount; j++) {
			...
			if (isPath(vidToLabel(u), vidToLabel(v))) {
				cout << vidToLabel(u) << " does have a path to " << vidToLabel(v) << endl;
			} else {
				cout << vidToLabel(u) << " does not have a path to " << vidToLabel(v) << endl;
			}
		}
	}
}

...
This .cpp file for the program creates a binary search tree and includes
functions to insert text in the tree, search the tree for a specific text,
remove text, find the value in the leftmost node, and so on and so forth.
A binary search tree is made up of nodes and pointers so those are in
here a lot.
*/
...
//Bella - a deconstructor for the sBST that clears every node and takes us back
//to the root
...
string sBST::findMin(sNode *ptr) const {
...
/*Bella - stub it out but dont let the stress leave you because its due in three
days
*/
...
/*Bella - stub this out too. again make sure to think about it every waking
minute so when its DUE you know what to DO (im not sorry)
*/
...
it works now. it deletes the sBST and clears everything out including the

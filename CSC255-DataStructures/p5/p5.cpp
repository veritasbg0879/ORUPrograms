...
/*Bella - balance the tree! If its LLheavy then rotate that sucker right. if its
LR heavy then rotate a subtree one way and rotate the main tree the other (same 
thing we do for RLheavy but reversed?) and if its RR heavy just swing it left. 
its pretty cool because everything just works so much better with balance.
*/
...
void sAVL::printIt(sNode *node, int &index) const {
    if (node){
        printIt(node->left, index);
    	cout << "At " << index << " the string is " << node->text <<
    	": height = " << node->h << endl;
	index++;
        printIt(node->right, index);
    }
}
...
//Bella - call the private printIt wooohooooo recursion

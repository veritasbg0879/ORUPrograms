...
to priority. make em directed and see if theyre cyclic and all kindsa stuff
*/
...
/*Bella
if its undirected just see if there's a path from u to itself. e z
*/

bool Graph::isCyclicUndirected() {
	bool rc = false;
	for (int u = 0; ((u < vCount) && (!rc)); u++) {
		for (int v = 0; ((v < vCount) && (!rc)); v++) {
			int uLabel = vidToLabel(u);
			int vLabel = vidToLabel(v);
			
			if(!isEdge(uLabel, uLabel)) {
				int weight;
				deleteEdge(uLabel, vLabel, weight);
				if (isPath(uLabel, vLabel)) {
					rc = true;
				}
...
/*Bella
if its directed see if theres a cycle. if we can get from u to v without
the edge then there's gotta be a cycle and therefore its cyclic
*/
...
/*Bella
use that good ol recursion we haven't seen it in a hot sec
*/
...
void Graph::printIt() const {
    ...
    for (int r = 0; r < sizeUsedV(); r++) {
        ...
        labels->readAt(r, label);
        cout << "  Node(" << r << "," << label << "):";
        cout << setw(6) << degree(label) << ",  " << inDegree(label) << ",  " 
		<< outDegree(label) << endl;
    }
